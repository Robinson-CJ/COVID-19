...
const line = function (id) {
  ...
  var myEcharts1 = echarts.init(dom)
  var option = {
    xAxis: {
      type: 'category',
...
const chinaMap = function (id, data) {
  ...
  const myEcharts2 = echarts.init(dom)
  const option = {
    //提示信息
    tooltip: {
      //事件类型
      ...
      formatter(data) {
        return `<div>
          <a href='/covid_19_dome/#/city/${data.name}' style='color:#fff'>
            <p>${data.name}</p>
            <p>现存确诊：${data.value}</p>
          </a>
        </div>`
    ...
    series: [
      {
        //地图的级别，这个级别的地图匹配的就是省份的数据，选中区域会显示
        name: '省',
        type: 'map',//配置图表类型
        map: 'china',//中国地图
        roam: true, //是否允许缩放
...
const cityMap = function (id, cityName, data) {
  const dom = documetn.getElementById(id)
  const myEcharts3 = echarts.init(dom)
  const option = {
    tooltip: {
      // 事件类型
      triggerOn: 'click',
    ...
    series: [
      {
        //地图的级别，中国级别的地图匹配JS省份的数据，选中区域会显示
        name: '省',
        type: 'map', //配置图表类型
        map: cityName, //必须写中文！
        roam: true,//是否允许缩放
...
const worldMap = function (id, data) {
  const dom = document.getElementById(id)
  const myEcharts4 = echarts.init(dom)
  const option = {
    // 提示信息
    tooltip: {
      // 事件类型
